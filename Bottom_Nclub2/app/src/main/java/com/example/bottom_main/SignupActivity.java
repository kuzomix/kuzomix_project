...
public class SignupActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        signupButton.setOnClickListener(v -> {
            ...
            String userId = databaseReference.push().getKey(); // 自動生成的使用者 ID
            ...
            reference.child(name).addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    if (snapshot.exists()) { // 檢查users中，name是不是已經被註冊過了
                        ...
                        HelperClass helperClass = new HelperClass(name, email, username, hashedPassword,createdAt, lastLogin, userId); // 建立HelperClass
